    ...
    while len(signals) > 1:
        ...
        for i in range(0, len(signals), 2):
            if i + 1 < len(signals):
                ...
                node = f"and_dtl{level}_{len(tree)}{label}"
...
def build_sarlock_logic(inputs, key_wires, target):
    ...
    for i, key in enumerate(key_wires):
        ...
        dtl0_inputs_X.append(in0)
        dtl0_inputs_Y.append(in1)

    dtl0_out_X, tree_X = build_tree(dtl0_inputs_X, label="_X")
    dtl0_out_Y, tree_Y = build_tree(dtl0_inputs_Y, label="_Y")

    dtl2_out = "DTL_2"
    logic += tree_X + tree_Y
    logic.append(f"{dtl2_out} = NAND({dtl0_out_Y}, {dtl0_out_Y})")
    logic.append(f"FLIP = AND({dtl0_out_X}, {dtl2_out})")
...
def main():
    parser = argparse.ArgumentParser(description="SARLock Logic Locking (Atalanta-compatible)")
